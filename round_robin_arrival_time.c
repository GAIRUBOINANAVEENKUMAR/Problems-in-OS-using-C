...
int main() {
    ...
    for (i = 0; i < n; i++) {
        printf("Enter the arrival time of process %c: ", pname[i]);
        scanf("%d", &arrival[i]);
    }
    char bur[5];
    for(int i=0;i<n;i++){
    ...
    printf("\nProcess Name\tArrival Time\tBurst Time\tWaiting Time\tResponse Time\tTurnaround Time\n");
    for (int i = 0; i < n; i++){
        printf("%c\t\t%3d\t\t%3d\t\t%3d\t\t%3d\t\t%3d\n", pname[i], arrival[i],burst[i], wait[i], res[i], tat[i]);
    }
    float r;
    float w;
    float turn;
